...
class BasinSetup(VerosSetup):
    ...
    def set_parameter(self, state):
        ...
        settings.restart_input_filename = "restarts/OverturningBeta4e5Coarse_1436400.restart.h5"
        settings.identifier = "Hosing"
        settings.description = "Uniform hosing 0.2 Sv north of 50 N. Implimented as just a salinity flux."
        settings.nx, settings.ny, settings.nz = 12, 18, 18

        settings.dt_mom = 3000
        settings.dt_tracer = 86400
        settings.runlen = 3600 * 24 * 30 * 12 * 420
        ...
        settings.kappaM_0 = 4e-5
        settings.kappaH_0 = 4e-5
    ...
    def set_diagnostics(self, state):
        ...
        diagnostics["overturning"].output_frequency = 24 * 3600 * 30 * 12 * 5  # 5 years
...
def set_forcing_kernel(state):
    ...
    # salt flux: m g/s kg
    A = 2 * npx.pi * settings.radius**2 * (npx.sin(npx.radians(65)) - npx.sin(npx.radians(50))) * 48 / 360
    ...
    vs.forc_salt_surface = - w * vs.salt[:, :, -1, vs.tau] * vs.maskT[:, :, -1]
